...
1. **圆轨迹数据不可用于拟合交叉项**。圆是 2D 空间里的**一维流形**——所有量都只是单个
   相位的函数，`q_other` 与本轴速度**同相位**（`q_fb=A·cosφ`、`q_lr=A·sinφ`、本轴速度
   `∝−sinφ`），故"静态交叉项 c(q_other)"与"速度项 Kv·q̇"**退化不可辨识**。
   rig2 实测：圆环拟合出交叉 `fb←lr=−1.008`，换真 2D 网格（grid/织网/8字）后变 **+0.833**
   —— **符号都翻了**，二次项也从 −0.106 崩到 ≈0。
   → **必须用真 2D 覆盖数据（`--grid` / 织网 / 8字），且收敛样本要够（圆环仅 3%，网格 30~42%）。**
2. **离线留出 RMSE 不是可靠的实机前馈判据**。它衡量"模型对稳态 (q,u) 的拟合优度"，
   加参数几乎必然改善；rig2 实测离线 −7.5% 通过护栏，**实机 A/B 却两轴劣化 +6.6~10.8%**。
   → 护栏必要但不充分，**实机 A/B 才是唯一裁判**。
3. **A/B 必须隔离唯一变量**：rig2 那次同时换了 own 形状与交叉项（联合拟合的 own ≠ 1D 的 own），
   劣化无法归因。→ 要测交叉项，就构造 `own=1D 系数 + cross=交叉项` 的混合配置。
